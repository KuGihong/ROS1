...
class PathPlanner:
    def __init__(self):
        ...
        self.goal_pose = Pose()
        self.goal_pose.position.x = 25.0
        self.goal_pose.position.y = -10.0
        ...
        #self.pub_command = rospy.Publisher("/cnu/cmd_vel", Twist, queue_size=1)
        self.target = Marker()
    ...
    def display_path(self, path_nodes: list) -> Path:
        ...
        '''
        del_x = target_x - self.cur_x
        del_y = target_y - self.cur_y
        dist = np.sqrt(del_x*del_x + del_y*del_y)
